...
void opticalFlow::runFlowEstimator(std::string i1file, 
    ...
    for (int i = 0; i < 2; ++i) {
        if (i == 0) { //optical flow from frame 1 to frame 2
            opticalFlowEst(im1, im2, flow12, params, 1);
        }
        else { //optical flow from frame 2 to frame 1
            opticalFlowEst(im2, im1, flow21, params, -1);
        }
    }
	...
	//cv::ximgproc::l0Smooth(flow_refined, flow_refined, 0.02, 2);
    MotionToColor(flow_refined, flow_color_t, -1);
    // cv::imshow("Flow aft Post-processing",flow_color_t);
    cv::imwrite(visualname, flow_color_t);
	writeToDepthImage(flow_refined, 3635.0f, 35.0f, depthname);
    flow_color_t.release();
}

int opticalFlow::writeToDepthImage(cv::Mat flow, float focal, float baseline, std::string name) {
	cv::Mat depthImg(flow.rows, flow.cols, CV_16U);
	for (size_t i = 0; i < depthImg.rows; i++) {
		for (size_t j = 0; j < depthImg.cols; j++) {
			cv::Point2f flowVal = flow.at<cv::Point2f>(i, j);
			float depthVal = baseline * focal / abs(flowVal.x);
				//sqrt(powf(flowVal.x, 2) + powf(flowVal.y, 2));
			depthImg.at<unsigned short>(i, j) = static_cast<unsigned short>(depthVal);
		}
	}
	cv::imwrite(name, depthImg);
	return 0;
}

void opticalFlow::opticalFlowEst(Mat& img1, Mat& img2, Mat_<Vec2f>& flow, spm_bp_params* params, int positive) {
    spm_bp* estimator = new spm_bp(positive);
...
void opticalFlow::opticalFlowRefine(Mat_<Vec2f>& flow_in, Mat_<uchar>& occMap, 
	const Mat_<Vec3b>& weightColorImg, Mat_<Vec2f>& flow_refined) {
...
void opticalFlow::WriteFlowFile(const char* flowFile, const cv::Mat_<cv::Vec2f>& flowVec,
